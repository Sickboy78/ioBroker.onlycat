...
class OnlyCatApi {
     ...
     * @param {(data: unknown) => void} callback the callback method to call when event is received
     */
    ...
    /* The gateway responses are schemaless JSON and are shaped by each caller, so 'any' is intentional here. */
     ...
     * @returns {Promise<any>} a response
     */
    async socketRequest(event, ...args) {
        ...
        return new Promise((resolve, reject) => {
            if (this.socket !== undefined) {
                ...
                const timeout = this.adapter.setTimeout(timeoutHandler, 30000);
